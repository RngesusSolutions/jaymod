...
def fetchRepoData():
    ...
    # Try to get SVN info, but don't fail if SVN is not available
    try:
        p = subprocess.Popen('svn info', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        p.wait()
        if (p.returncode != 0):
            # SVN not available or not an SVN repository - return default values
            return data
        ...
        # If anything goes wrong, just return default values
        pass
